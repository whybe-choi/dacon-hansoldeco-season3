...
(4) 굴착작업 진행 시 일반 슬래브는 토압, 수압 등의 환력에 버팀 역할을 하나, 램프 슬래브는 상하층 경사로로 시공되어 구조물이 완성되기 전까지는 환력에 대한 버팀대의 역할을 하지 못하므로 환력에 저항토록 버팀대(Strut)등을 별도로 설치하여 붕괴 및 변형을 방지하도록 하여야 한다.
